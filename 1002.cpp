...
int main() {
  setIO();
	...
	unordered_map<char, str> dec;
	vs enc = {"oqz", "ij", "abc", "def", "gh", "kl", "mn", "prs", "tuv", "wxy"};
	FOR(i,0,10) {
		dec[i + '0'] = enc[i];
	}
  ...
  while (num != "-1") {
		auto good = [&](str w, int ind, int ln) {
			str ch = num.substr(ind, ln);
			FOR(i,0,sz(w)) {
				if (dec[ch[i]].find(w[i]) == str::npos) return false;
			}
			return true;
		};
    int n; cin >> n;
    vs words(n); cin >> words;
    int m = sz(num);
    vi dp(m + 1, INF);
    vs par(m + 1);
    dp[0] = 0;
    FOR(i,1,m+1) {
      for (auto w : words) {
        if (sz(w) > i) continue;
        if (good(w, i - sz(w), i) && dp[i - sz(w)] < INF && dp[i] > dp[i - sz(w)] + 1) {
          dp[i] = dp[i - sz(w)] + 1;
					par[i] = w;
        }
      }
