...
int main() {
  setIO();
	...
	int n, m; cin >> n >> m;
  set<vi> trip;
  FOR(i,1,100) FOR(j,i,100) FOR(k,j,100) {
    if (i * i + j * j == k * k) {
      trip.insert({i * i, j * j, k * k});
    }
  }
  vs g(n); cin >> g;
  FOR(i,0,n) FOR(j,0,m) {
    int v; cin >> v;
    if (v % 2 == 0) continue;
    FOR(i2,0,n) FOR(j2,0,m) {
      int x = abs(i - i2), y = abs(j - j2);
      if (x > y) swap(x, y);
      int z = x * x + y * y;
      if (trip.count({x * x, y * y, z}) || x == 0 || y == 0) {
        g[i2][j2] = g[i2][j2] == 'B' ? 'W' : 'B';
      }
    }
  }
  for (auto x : g) {
