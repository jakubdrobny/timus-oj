...
int main() {
  ...
  const int mxk = 35;
	int n; cin >> n;
  vector<vector<vector<pair<int, int>>>> g(n+1, vector<vector<pair<int, int>>>(mxk));
  FOR(i,0,n) {
    int l; cin >> l;
    FOR(j,0,l) {
      int planet, w; cin >> planet;
      while (planet != 0) {
        cin >> w;
        g[i+1][j].pb(mp(planet, w));
        cin >> planet;
      }
    }
    char ch;
  ...
  FOR(i,1,n+1) {
    FOR(j,0,mxk) {
      if (sz(g[i][j])) {
        for (auto p : g[i][j]) {
          ckmin(dp[i][j], dp[i-1][p.fi-1]+p.se);
          if (i == n) {
            ckmin(ans, dp[i][j]);
          }
        }
