...
int main() {
  ...
  int n; cin >> n;
  vector<pair<pair<pii, pii>, int>> sq(n);
  FOR(i,0,n) {
    cin >> sq[i].fi;
    if (sq[i].fi.fi.fi > sq[i].fi.se.fi) {
      swap(sq[i].fi.fi, sq[i].fi.se);
    }
    sq[i].se = i;
  }
  pii p; cin >> p;
  auto manhat = [&](auto pp) {
    double x = pp.fi - p.fi, y = pp.se - p.se;
    return sqrt(x * x + y * y);
  };
  auto getd = [&](auto t) {
    pii l = t.fi.fi, r = t.fi.se;
    if (p.fi >= l.fi && p.fi <= r.fi && p.se >= l.se && p.se <= r.se) {
      return 0.;
    }
    double res = INF;
    for (auto pp : pps) {
      double cur = manhat(pp);
      if (res - cur < eps) {
        res = cur;
      }
    }
  ...
  sort(all(sq), [&](auto a, auto b){
    double gda = getd(a), gdb = getd(b);
    if (abs(gda - gdb) < eps) {
      return a.se < b.se;
    }
    return gdb - gda > eps;
