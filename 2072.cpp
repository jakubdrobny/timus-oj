...
int main() {
  ...
  ll l = 0, r = 1;
  dp[0][r] = a[0].se.back();
  dp[0][l] = (dp[0][r] << 1) - a[0].se[0];
  FOR(i,1,n) {
    ll c = abs(a[i].se.back() - a[i].se[0]);
    dp[i][l] = min(
      dp[i-1][l] + abs(a[i-1].se[0] - a[i].se.back()) + c,
      dp[i-1][r] + abs(a[i-1].se.back() - a[i].se.back()) + c
    );
    dp[i][r] = min(
      dp[i-1][l] + abs(a[i-1].se[0] - a[i].se[0]) + c,
      dp[i-1][r] + abs(a[i-1].se.back() - a[i].se[0]) + c
    );
  }
  cout << min(dp[n-1][0],dp[n-1][1]) + on << "\n";
