...
int main() {
  setIO();
	...
	dp[0][0] = 0;
	FOR(i,0,S) FOR(j,0,S2) {
		if (dp[i][j] == -1) continue;
		FOR(d,1,10) {
			if (i + d > S || j + d * d > S2) break;
			if (dp[i + d][j + d * d] == -1 || dp[i + d][j + d * d] > dp[i][j] + 1) {
				dp[i + d][j + d * d] = dp[i][j] + 1;
			}
		}
	}
	...
	manytests {
		int s, s2; cin >> s >> s2;
		if (s > 900 || s2 > 8100 || dp[s][s2] == -1 || dp[s][s2] > 100) {
			cout << "No solution\n";
		} else {
			while (s > 0) {
				FOR(d,1,10) {
					if (s - d >= 0 && s2 - d * d >= 0 && dp[s - d][s2 - d * d] + 1 == dp[s][s2]) {
						s -= d;
						s2 -= d * d;
						cout << d;
						break;
